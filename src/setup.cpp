...
void main_setup() { // dam break; required extensions in defines.hpp: FP16S, VOLUME_FORCE, SURFACE, INTERACTIVE_GRAPHICS
	...
	parallel_for(lbm.get_N(), [&](ulong n) {
		...
		// 水柱：z < 6/8 Nz 且 y < Ny/8
		if (z < Nz * 6u / 8u && y < Ny / 8u) {
			lbm.flags[n] = TYPE_F;
			lbm.phi[n] = 1.0f;  // 设置填充水平为满
		}
		else {
			lbm.flags[n] = TYPE_G;  // 气体单元
			lbm.phi[n] = 0.0f;
		}
	...
	for (uint step = 0u; step < max_steps; step += log_interval) {
		lbm.run(log_interval);

		// 计算当前总质量 - 通过 lbm.lbm_domain[0] 访问
	...
	// 如果无限运行，改成 while(true) { lbm.run(log_interval); ... }
}
